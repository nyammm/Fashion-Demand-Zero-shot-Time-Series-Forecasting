...
def calculate_metrics(predict, label):
    ...
    mae = mae(label, predict)
    # mae = np.mean([mae(label[i,:], predict[i,:]).cpu().item() for i in range(label.size(0))])
    ...
    return mae, np.mean(wmape), np.mean(adj_smape), accum_smape.cpu().item()


def load_csv():
    train_data = pd.read_csv(os.path.join(args.data_folder + 'stfore_train.csv'), parse_dates=['release_date'])  
    test_data = pd.read_csv(os.path.join(args.data_folder + 'stfore_test.csv'), parse_dates=['release_date'])
...
def compute_batch_similarity(tags, num):
    ...
    for i in range(0, tag.size(0), batch_size):
        ...
        print("[tag {}] Processing: {}/{}".format(num, batch_size, tag.size(0)))
...
def compute_similarity(tags, codes, img_emb, mode="tags"):
    ...
    if mode == "tags":
        tags = torch.tensor(tags, device=device).half()
        ...
        similarity_matrix = similarity_matrix.squeeze(-1).to(torch.float32)
        similarity_matrix /= 3
        print("Tag similarity is ready")
...
def run(args):
    ...
    args.fclip_image = "fclip_img.pkl"
    ...
    if args.exp_num == 1:
        similarity_matrix = compute_similarity(tags, img_paths, img_emb, mode="tags")
    elif args.exp_num == 2:
    ...
    print(f"MAE: {mae:.4f}\nWMAPE: {wmape:.4f}\nadj_SMAPE: {adj_smape:.4f}\naccum_SMAPE: {accum_smape:.4f}")
    print("-----------------------------------------------------------------------------------------------------------")
