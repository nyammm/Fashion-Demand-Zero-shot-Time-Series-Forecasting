...
def trainModel(model, train_dataloader):
   ...
   for epoch in range(config.NUM_EPOCHS):
      ...
      for i, (images, trend, categ, color, fabric, _, release_date, temporal_features, img_feature, _) in enumerate(train_dataloader):
         ...
         outputs = outputs * normalization_values
         trend = trend * normalization_values

         # Forward, backward and optimize
         outputs = outputs.to(device)
         loss = criterionL1(outputs, trend)
         loss.backward()
         avg_batch_wmape = torch.sum(torch.sum(torch.abs(trend - outputs), dim=-1)) / torch.sum(torch.sum(trend, dim=-1))
         batch_adj_smape = torch.sum(torch.sum(torch.abs(trend - outputs), dim=-1) / torch.sum(torch.abs(trend) + torch.abs(outputs), dim=-1)) / config.BATCH_SIZE
         batch_accum_smape = torch.sum(torch.abs(torch.sum(trend, dim=-1) - torch.sum(outputs, dim=-1)) / torch.sum(torch.abs(trend) + torch.abs(outputs), dim=-1)) / config.BATCH_SIZE
         model_optimizer.step()
         ep_loss += loss.item()

         # Print log info
         if i % 10 == 0: 
            logging.info('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}, MAE: {:.4f}, adj-SMAPE: {:.4f}, accum-SMAPE: {:.4f}, wMAPE: {:.4f}'.format(epoch + 1, config.NUM_EPOCHS, i, total_step, criterion(outputs, trend).detach().cpu(), loss.item(), batch_adj_smape.item(), batch_accum_smape.item(), avg_batch_wmape.item()))
      epoch_score = evaluate(model, test_dataloader)
