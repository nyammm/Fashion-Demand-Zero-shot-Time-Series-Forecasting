...
def evaluate(model, test_dataloader, show_plots=False):
   ...
   with torch.no_grad():
      for index, elem in enumerate(test_dataloader):
         ...
         if show_plots:
            ...
            for ii in range(out_norm.shape[0]):
               ...
               wMAPE = torch.sum(torch.abs(trend - outputs), dim=-1) / torch.sum(gts)
               plt.plot(res)
               plt.plot(gt)
      ...
      # wMAPE = torch.sum(torch.sum(torch.abs(trend - outputs), dim=-1)) / torch.sum(torch.abs(outputs))
      wMAPE = torch.sum(torch.abs(trend - outputs), dim=-1) / torch.sum(outputs, dim=-1)
...
# train_dir_name = os.path.join(model_filename)
logging.info("Evaluating")
best_model_file = find_model_file('image_lr0.0001_tf_05_exog_52_datas_10ago_11-11-2024-12-43')
