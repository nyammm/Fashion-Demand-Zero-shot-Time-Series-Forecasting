...
class DecoderRNN(nn.Module):
    ...
    def forward(self, x_input, decoder_hidden):
        ...
        # output = self.leaky_relu(linear_out)
...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, attention_dim, image_feature_size, hidden_size, encoder, decoder, out_len = 12, use_teacher_forcing = False):
        ...
        self.category_embeds = nn.Embedding(27, self.attr_emb_size)
        self.color_embeds = nn.Embedding(16, self.attr_emb_size)
        self.fabric_embeds = nn.Embedding(7, self.attr_emb_size)
    ...
    def forward(self, input_batch, category, color, fabric, temporal_info, exogeneous_params, target=None, img_feature=None):
        ...
        # print(attributes_embedding.shape)
        ...
        for t in range(self.out_len):
            ...
            # Attention
            attention_weighted_encoding, alpha = self.attention(torch.cat([inception_feat, attributes_embedding], dim=2), decoder_hidden)
            ...
            if config.model_types[config.MODEL] == "residual":
                ...
                residual_block = attributes_embedding + weight_cat_embedding + attention_weighted_encoding 

                x_input = [residual_block, decoder_output]
            
            #Concat
            else:
                x_input = [attention_weighted_encoding, attributes_embedding, decoder_output]
