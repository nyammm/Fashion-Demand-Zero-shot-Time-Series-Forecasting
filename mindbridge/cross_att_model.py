...
class EncoderDecoder(nn.Module):
    def __init__(self, l_seq, device, use_teacher_forcing=True):
        ...
        self.category_embeds = nn.Embedding(27, self.D)
        self.color_embeds = nn.Embedding(16, self.D)
        self.fabric_embeds = nn.Embedding(7, self.D)
    ...
    def forward(self, input_batch: torch.FloatTensor, categ: torch.LongTensor, color: torch.LongTensor, fabric:  torch.LongTensor, temporal_info: torch.FloatTensor, exogeneous_params: torch.LongTensor,
        ...
        for t in range(self.l_seq):
            attention_weighted_encoding, _ = self.img_attention(
                self.proj_img_att_concat(torch.cat([inc_feats, attr_embedding.unsqueeze(1)], dim=2)),
                decoder_hidden)
            ...
            if config.USE_EXOG:
                ...
                J = torch.cat(
                    [attention_img_embedding.unsqueeze(1), attr_embedding.unsqueeze(1), gtrend_embedding.unsqueeze(1)],
                    dim=1)
